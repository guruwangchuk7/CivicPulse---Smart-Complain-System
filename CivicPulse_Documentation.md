...
- **Centralizing Infrastructure Data**: Creating a single source of truth for all reported issues.
- **Enabling Instant Reporting**: Allowing users to report issues in seconds via a mobile-responsive map.
- **Improving Transparency**: Letting citizens track the status of their reports in real-time.
...
4. **Submit**: Confirm the location and submit.
5. **Track**: Revisit the marker on the map to see if the status has shifted from "Open" to "Pending" or "Resolved".
...
4. **Update**: Mark the issue as "Pending" once work begins and "Resolved" once completed.
...
- **API Architecture**: Next.js Serverless Routes for:
    - `/api/reports`: Handles GET (listing/filtering) and POST (creating).
